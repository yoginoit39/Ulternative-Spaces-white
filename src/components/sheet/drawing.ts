...
export function buildDrawing(category = 'Residential', seed = 7): Drawing {
  ...
  note.push({ d: `M ${PX1 - 4} 128 L ${PX1 + 8} 166 L ${PX1 - 4} 158 L ${PX1 - 16} 166 Z`, layer: 'note', w: 0.8, fill: 'rgba(var(--fg-rgb),.25)' });
  // scale bar
  ...
  note.push({ d: R(sbx, sby - 3, 40, 3), layer: 'note', w: 0.3, fill: 'rgba(var(--fg-rgb),.3)' });
  note.push({ d: R(sbx + 80, sby - 3, 40, 3), layer: 'note', w: 0.3, fill: 'rgba(var(--fg-rgb),.3)' });
